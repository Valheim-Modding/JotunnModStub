...
# (e.g. ./start_server_bepinex.sh -name "MyVeilheimServer" -password "somethingsafe" -port 2456 -world "myworld" -public 1)
server_name="Veilheim"
server_password="password"
server_port=2456
server_world="world"
server_public=1

# The rest is automatically handled by BepInEx for Veilheim

# Set base path of start_server_bepinex.sh location
export VEILHEIM_SCRIPT="$(readlink -f "$0")"
export VEILHEIM_PATH="$(dirname "$VEILHEIM_SCRIPT")"

# Whether or not to enable Doorstop. Valid values: TRUE or FALSE
export DOORSTOP_ENABLE=TRUE

# What .NET assembly to execute. Valid value is a path to a .NET DLL that mono can execute.
export DOORSTOP_INVOKE_DLL_PATH="${VEILHEIM_PATH}/BepInEx/core/BepInEx.Preloader.dll"

# Which folder should be put in front of the Unity dll loading path
export DOORSTOP_CORLIB_OVERRIDE_PATH="${VEILHEIM_PATH}/unstripped_corlib"

# ----- DO NOT EDIT FROM THIS LINE FORWARD  ------
# ----- (unless you know what you're doing) ------

if [ ! -x "$1" -a ! -x "${VEILHEIM_PATH}/$executable_name" ]; then
	echo "Please open start_server_bepinex.sh in a text editor and provide the correct executable."
	exit 1
fi

doorstop_libs="${VEILHEIM_PATH}/doorstop_libs"
arch=""
executable_path=""
lib_postfix=""
...
case $os_type in
	Linux*)
		executable_path="${VEILHEIM_PATH}/${executable_name}"
		lib_postfix="so"
		;;
	Darwin*)
		executable_name="$(basename "${executable_name}" .app)"
		real_executable_name="$(defaults read "${VEILHEIM_PATH}/${executable_name}.app/Contents/Info" CFBundleExecutable)"
		executable_path="${VEILHEIM_PATH}/${executable_name}.app/Contents/MacOS/${real_executable_name}"
		lib_postfix="dylib"
		;;
	*)
...
export LD_LIBRARY_PATH="${VEILHEIM_PATH}/linux64":"${LD_LIBRARY_PATH}"
...
"${VEILHEIM_PATH}/${executable_name}" -name "${server_name}" -password "${server_password}" -port "${server_port}" -world "${server_world}" -public "${server_public}"
